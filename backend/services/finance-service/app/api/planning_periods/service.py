    ...
    async def create_period(db: AsyncSession, data: dict, user_id: str) -> dict:
        ...
        if source_period_id:
            ...
            for sl in source_lines:
                # Remap month from source period range to new period range
                src_month = int(sl.period.split("-")[1])
                # Only copy if month is within new period range
